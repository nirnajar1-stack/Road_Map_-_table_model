...
import { FolderKanban, Map, Plus, Trash2 } from "lucide-react";
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-theme-page">
      <header className="nav-transparent">
        <div className="max-w-content mx-auto px-10 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Map size={28} className="text-lambo-gold" strokeWidth={1.5} />
            <div>
              <h1 className="heading-display text-2xl sm:text-3xl">ROAD MAP</h1>
              <p className="label-micro mt-1">מפת דרכים מודולרית</p>
            </div>
          </div>
        ...
        {projects.length === 0 ? (
          <div className="text-center py-24">
            <div className="inline-flex p-6 bg-theme-surface mb-8 border border-theme-border">
              <FolderKanban size={48} className="text-theme-subtle" strokeWidth={1} />
            </div>
            ...
            <p className="text-theme-muted text-base max-w-md mx-auto mb-10 leading-relaxed">
              צור את הפרויקט הראשון שלך והתחל לבנות מפת דרכים מודולרית עם שלבים
              ונקודות ציון מתוזמנות
            </p>
          ...
          <>
            <div className="mb-10">
              <p className="label-caption mb-2">הפרויקטים שלי</p>
                ...
                >
                  ...
                  <Link href={`/project/${project.id}`} className="block p-8 pr-10">
                    ...
                    <div className="flex items-center gap-6 mt-6 label-micro">
                      <span>{project.stages.length} שלבים</span>
                      <span>
                        {project.stages.reduce((sum, s) => sum + s.milestones.length, 0)} נקודות
                      </span>
      ...
      {showForm && (
        <Modal title="פרויקט חדש" onClose={() => setShowForm(false)}>
          <ProjectForm
